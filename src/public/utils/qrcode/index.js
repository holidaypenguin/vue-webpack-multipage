...
const api = {
  ...
  draw (string, canvas, size, ecc) {
    ...
    // draw: function (string, { ctx, w, h, fg = "#000000", bg = "#ffffff" } = _canvas, size, ecc = ecclevel) {
    ...
    const offsetNum = 1
    const ctx = isWx ? canvas.ctx : canvas.ctx.getContext('2d')
    const px = Math.round(size / (width + offsetNum * 2))

    const roundedSize = px * (width + offsetNum * 2)
    let offset = Math.floor((size - roundedSize) / 2)

    size = roundedSize

    if (!isWx) {
      canvas.ctx.style.width = `${size}px`
      canvas.ctx.style.height = `${size}px`
      canvas.ctx.setAttribute('width', size)
      canvas.ctx.setAttribute('height', size)
      offset = 0
    }

    ctx.clearRect(0, 0, canvas.width, canvas.height)
    setFillStyle(ctx, canvas.bg)
    ctx.fillRect(0, 0, canvas.width, canvas.height)

    setFillStyle(ctx, canvas.fg)

    for (let i = 0; i < width; i++) {
      for (let j = 0; j < width; j++) {
        if (frame[j * width + i]) {
          // ctx.fillRect(px * (4 + i) + offset, px * (4 + j) + offset, px, px);
          ctx.fillRect(px * (offsetNum + i) + offset, px * (offsetNum + j) + offset, px, px)
