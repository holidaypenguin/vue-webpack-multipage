...
iSlider.prototype = {
  ...
  init () {
    ...
    if (this.SS) {
      ...
      // const lastLocateIndex = parseInt(this.SS[this._sessionKey], 10)
      const lastLocateIndex = 0
    ...
    for (let i = 0; i < this._tpl.length; i++) {
  ...
  _bindEvt () {
    const self = this
    const handlrElm = this.opts.fullScr ? this.$('body') : this.wrap
  ...
  _setHTML (index) {
    ...
    const initDom = document.createDocumentFragment()
  ...
  _touchmove (e) {
    ...
    const gx = Math.abs(e.touches[0].pageX - this._touchstartX)
    const gy = Math.abs(e.touches[0].pageY - this._touchstartY)
    ...
    const currentX = this.opts.isVertical ? e.touches[0].pageY : e.touches[0].pageX
  ...
  _getTransform (dist) {
    const pos = this.opts.isVertical ? `0,${dist}` : `${dist},0`
  ...
  _loading () {
    const self = this
    const imgurls = this.opts.loadingImgs
